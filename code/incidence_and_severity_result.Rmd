...
```
We could do a model with a interaction with all the variable (Shade_tree_species * distance * Coffee_Variety). It work but a 3 interaction term require to estimate à large quantity of parameter. I don't think we have enough data (of incidence) to estimate well those parameters.
...
ggeffects::predict_response(mod_binary_incidence_CWD2, c("distance[all]"), margin = "empirical") %>%
  plot(show_data =T,show_ci=T, show_title = F,
        dot_size  = 3, 
        dot_alpha = 0.4,
        line_size  = 1.5) +
...
modelbased::estimate_means(mod_binary_incidence_CWD2, by = c("distance", "Shade_tree_species"), estimate ="typical", backend = "marginaleffects") %>% 
  as.data.frame() %>%
  ggplot()+
  ...
  geom_point(data = binary_data_CWD, aes(distance,true_incidence, fill = Shade_tree_species),
             alpha = 0.8, shape = 21, cex = 3, color="black",show.legend = F)+
  facet_grid(row = vars(Shade_tree_species), switch="both")+
  scale_y_continuous(limits = c(0,1))+
  labs(x = "Distance from\n shade tree (m)", y = "CWD incidence probability") +
  ...
  theme(aspect.ratio = 1,
        ...
        legend.position = "None",
        panel.border = element_rect(colour = "black", fill = NA, linewidth = 1),
        axis.title.y = element_text(colour = "black", size=14, face="italic")) -> p6
p6
...
p6+theme(axis.title.x =  element_text(size=16,face="bold"),
  ...
  plot_annotation(tag_levels = 'A') -> plot_cwd_incidence
plot_cwd_incidence
```
...
data_incidence %>%
 mutate(disease = case_when(disease == "CRBD" ~ "BES",
  ...
  labs(x = " Shade tree species", y = "Proportion of coffee shrubs", fill = "Symptoms\n incidence") +
  theme_minimal()+
  theme(aspect.ratio = 1,
      axis.line = element_line(colour = "black"),
      panel.border = element_blank(),
      ...
      axis.title= element_text(size=16,face="bold"),
      strip.text = element_text(colour = "black", size=15, face ="italic")) -> disease_incidence
...
#       axis.text.x = element_text(colour = "black", size=13, face="italic"),
#       axis.text.y = element_text(colour = "black", size=13, face="italic"),
#       legend.title = element_text(colour = "black", size=18, hjust =0.5,face="italic"),
#       legend.text = element_text(colour = "black", size=13),
#       axis.title= element_text(size=16,face="bold"),
#       strip.text = element_text(colour = "black", size=15, face ="italic")) 
disease_incidence
...
CWD_top%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + 
                                            (1|Shade_tree_soil_sample_CODE)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CWD_top6
CWD_top%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance + Coffee_Variety+
                                            (1|Shade_tree_soil_sample_CODE)"),
                              zifrml = formula("~Coffee_Variety"),
...
```{r}
AIC(mod_severity_CLR_low8,mod_severity_CLR_low10mod_severity_CLR_low11)
# Anova
...
clr_trend_shade %>%
  ...
  #facet_grid(rows = vars(type), scales = "free_y")+
  labs(x= "Estimated slopes for CLR severity", y = "Slopes types", col = "Predictors")+
...
CRBD_sev_var_temp %>%
  ...
  geom_violin(data = data_severity %>%
   ...
   mutate(disease = case_when(disease == "CRBD" ~ "BES",
                             .default = disease),
          disease = factor(disease, levels = c("CLR","BES", "CWD"))), aes(x = Coffee_Variety, y=severity, fill = disease, col = disease), 
...
ggsave(plot = plot_cwd_incidence, file="results/figures/plot_cwd_incidence.pdf",
width=25, height=15,units ="cm")
ggsave(plot = disease_incidence, file="results/figures/disease_incidence_shade_tree.pdf",
...
width=20, height=15, units ="cm")
ggsave(plot = severity_variety, file="results/figures/severity_variety.pdf",
width=20, height=15, units ="cm")
```
# Figures appendix
```{r}
pdf(file ="results/figures/res_incidence.pdf",  
    width = 10,
    height = 6) 
res_incidence <-DHARMa::simulateResiduals(mod_binary_incidence_CWD2, plot = T)
dev.off()

pdf(file ="results/figures/res_severity_CRBD.pdf",  
    width = 10,
    height = 6) 
res_severity_CRBD <- DHARMa::simulateResiduals(mod_severity_CRBD_leaves_top6, plot = T)
dev.off()

pdf(file ="results/figures/res_severity_CWD.pdf",  
    width = 10,
    height = 6) 
res_severity_CWD <- DHARMa::simulateResiduals(mod_severity_CWD_top8, plot = T)
dev.off()

pdf(file ="results/figures/res_severity_CLR.pdf",  
    width = 10,
    height = 6) 
res_severity_CLR <- DHARMa::simulateResiduals(mod_severity_CLR_low8, plot = T)
dev.off()
...
pdf(file ="results/figures/zero_inflation_test.pdf",  
    ...
    height = 6) 
par(mfrow = c(2,2))
testZeroInflation(res_incidence)
title("Incidence CWD")
