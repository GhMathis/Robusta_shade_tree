...
pwc_CWD_inc <- emmeans(mod_binary_incidence_CWD, ~Coffee_Variety, component = 'response')
pairs(pwc_CWD_inc)
multcomp::cld(pwc_CWD_inc, Letter = letters,decreasing = T) %>%
  as.data.frame() %>%
...
predict_response(mod_severity_CRBD_leaves_top6, c("Coffee_Variety"), type = "zero_inflated", margin = "empirical") -> CRBD_severity_variety
...
CWD_top%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*(distance + Coffee_Variety) +Coffee_Variety:distance+
                                            (1|Shade_tree_soil_sample_CODE)"),
                              zifrml = formula("~Coffee_Variety"),
                              disprml= formula("~Coffee_Variety"))-> mod_severity_CWD_top9
```
...
pwc_cwd <- emmeans(mod_severity_CWD_top4, ~Coffee_Variety, component = 'response')
...
predict_response(mod_severity_CWD_top4, c("Coffee_Variety"), type = "response", margin = "empirical") -> CWD_severity_variety
...
CWD_severity_variety%>%
   plot(show_data =F,show_ci=T, show_title = F,
        ...
        dot_alpha = 0.03,
        line_size  = 1) +
  geom_text(data = compl_letter_cwd_sev, aes(x = x, y =0.05, label = .group), vjust = 0)+
...
log(total leaf) mandatory to correct dispertion
-   **Model:**
...
```{r}
AIC(mod_severity_CLR_low10,mod_severity_CLR_low10)
# Anova
...
```{r}
predict_response(mod_severity_CLR_low10, c("distance[all]", "Shade_tree_species","Coffee_Variety"),
...
predict_response(mod_severity_CLR_low10, c("distance[all]", "Shade_tree_species"), 
...
predict_response(mod_severity_CLR_low10, c("distance[all]", "Coffee_Variety"),
...
predict_response(mod_severity_CLR_low10, c("distance[all]"),
...
```{r}
pwc_clr_sev <- emmeans(mod_severity_CLR_low10, ~Coffee_Variety, component = 'response')
pairs(pwc_clr_sev)
...
predict_response(mod_severity_CLR_low10, c("Coffee_Variety"), margin = "empirical", type = "zero_inflated") -> CLR_severity_variety
...
```{r}
pwc_clr_sev2 <- emmeans(mod_severity_CLR_low10, ~Shade_tree_species, component = 'response')
pairs(pwc_clr_sev2)
...
predict_response(mod_severity_CLR_low10, c("Shade_tree_species"), margin = "empirical", type = "zero_inflated") %>%
  plot(show_data =F,show_ci=T, show_title = F,
        dot_size  = 2, 
        dot_alpha = 0.03,
        line_size  = 1) +
...
```{r}
#Slope contrast
test_predictions(mod_severity_CLR_low10, c("distance", "Shade_tree_species"), margin = "empirical", test = "trend") %>%
  as.data.frame %>%
  rename(factors = "Shade_tree_species") %>%
  mutate(type = "Shade tree : Distance") -> clr_trend_shade

test_predictions(mod_severity_CLR_low10, c("distance", "Coffee_Variety"), margin = "empirical", test = "trend") %>%
  as.data.frame %>%
  rename(factors = "Coffee_Variety") %>%
  mutate(type = "Coffee variety : Distance") -> clr_trend_var

test_predictions(mod_severity_CLR_low10, c("distance"), margin = "empirical", test = "trend") %>%
  as.data.frame %>%
...
clr_trend_shade %>%
  ...
  mutate(factors = factor(factors, levels = c(as.character(clr_trend_shade$factors),
         ...
         type = factor(type, levels = c("Distance alone",
                                        ...
                                        "Shade tree : Distance"))) %>%
  ggplot() +
  geom_pointrange(aes(y = factors, x = Slope, xmin = conf.low, xmax = conf.high, colour = type),cex = 0.7, linewidth = 1) +
...
p_CRBD_severity_variety + theme(axis.title.x = element_blank(), axis.text.x= element_blank()) +
  ...
  plot_layout(nrow=3)
severity_variety
temp_plt +severity_variety & plot_annotation(tag_levels = 'A')
p_crl_trend 
p_CLR_severity_shade 
