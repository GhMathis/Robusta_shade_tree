...
Try different model, the only constrain is that we keep "distance*Shade_tree_species + (1|Section_of_Kaweri/Shade_tree_soil_sample_CODE)" .
...
# data_yield2 %>%
#   yield.mod.func2(frml = "berries_mean_per_branch ~ distance*Shade_tree_species + (1|Section_of_Kaweri/Shade_tree_soil_sample_CODE)",
#                     zifrml = "~0",
#                     dispfrml = "~ 1",
#                     fam = poisson)  -> mod_yield
# data_yield2 %>%
#   yield.mod.func2(frml = "berries_mean_per_branch ~ distance*Shade_tree_species + Coffee_Variety +
...
# 
# data_yield2 %>%
#   yield.mod.func2(frml = "berries_mean_per_branch ~ distance*Shade_tree_species + distance*Coffee_Variety +
#                     (1|Section_of_Kaweri/Shade_tree_soil_sample_CODE)",
#                     zifrml = "~0",
#                     dispfrml = "~ 1",
...
# data_yield2 %>%
#   yield.mod.func2(frml = " berries_mean_per_branch ~ distance * Shade_tree_species + distance * Coffee_Variety + CLR_leaf_incidence + CRBD_leaf_incidence + CWD_leaf_incidence + (1 | Shade_tree_soil_sample_CODE)",
#                     zifrml = "~0",
#                     dispfrml = "~ 1",
...
full_berrie_model <-  glmmTMB(formula = berries_mean_per_branch ~ distance*Shade_tree_species + distance*Coffee_Variety  + (CLR_leaf_incidence + CRBD_leaf_incidence+ CWD_leaf_incidence)*Coffee_Variety  + Section_of_Kaweri +
...
```{r}
MuMIn::get.models(dd_berrie, 1)[[1]]
```
...
marginaleffects::avg_slopes(selected_berrie_model,variables  = "distance", by = "Shade_tree_species", newdata = "balanced")  %>%
  as.data.frame %>%
  rename(factors = "Shade_tree_species") %>%
  mutate(type = "Shade tree : Distance") -> yield_trend_shade

marginaleffects::avg_slopes(selected_berrie_model,variables  =  "distance", by = "Coffee_Variety", newdata = "balanced") %>%
  as.data.frame %>%
  rename(factors = "Coffee_Variety") %>%
  mutate(type = "Coffee variety : Distance") -> yield_trend_var

marginaleffects::avg_slopes(selected_berrie_model,variables  = "distance", newdata = "balanced") %>%
  as.data.frame %>%
...
marginaleffects::avg_slopes(selected_berrie_model, variables = "CLR_leaf_incidence", by = "Coffee_Variety", newdata = "balanced") %>%
  as.data.frame %>%
  rename(factors = "Coffee_Variety") %>%
  mutate(type = "Coffee variety : CLR leaf incidence") -> yield_trend_CLR_var

marginaleffects::avg_slopes(selected_berrie_model, variables = "CRBD_leaf_incidence", by = "Coffee_Variety", newdata = "balanced") %>%
  as.data.frame %>%
  rename(factors = "Coffee_Variety") %>%
  mutate(type = "Coffee variety : BES leaf incidence") -> yield_trend_CRBD_var

marginaleffects::avg_slopes(selected_berrie_model, variables = "CWD_leaf_incidence", by = "Coffee_Variety", newdata = "balanced") %>%
  as.data.frame %>%
  rename(factors = "Coffee_Variety") %>%
  mutate(type = "Coffee variety : CWD leaf incidence") -> yield_trend_CWD_var

marginaleffects::avg_slopes(selected_berrie_model, variables = "CLR_leaf_incidence", newdata = "balanced") %>%
  as.data.frame %>%
  mutate(type = "Disease leaf incidence alone",
         factors ="CLR") -> yield_trend_CLR

marginaleffects::avg_slopes(selected_berrie_model, variables = "CWD_leaf_incidence", newdata = "balanced") %>%
  as.data.frame %>%
  mutate(type = "Disease leaf incidence alone",
         factors ="CWD") -> yield_trend_CWD

marginaleffects::avg_slopes(selected_berrie_model, variables = "CRBD_leaf_incidence", newdata = "balanced") %>%
  as.data.frame %>%
  mutate(type = "Disease leaf incidence alone",
...
marginaleffects::avg_comparisons(
  selected_berrie_model,
  variables = list(Section_of_Kaweri = "pairwise"),
  newdata = "balanced"
) -> contrast_section_df

get_significance_letters2(contrast_section_df) %>%
...
data_yield2 %>%
  ggplot( aes(x = Section_of_Kaweri, y = berries_mean_per_branch)) +
  # Add base data
  geom_violin(fill= "gray",color = "white") +
  #geom_jitter(width = 0.1, height = 0, alpha = 0.5, size = 3) +
  # Add pointrange and line for means
  geom_text(data = compl_letter_yield_section, aes(x = x, y =178, label = .group), vjust = 0)+
  geom_pointrange(
    data = yield_section,
    aes(y = estimate, ymin = conf.low, ymax = conf.high),
    size = 1,
  ...
  theme_minimal()+
   labs(x= "Sections", y = "Mean fruit load per branch")+
  theme_classic()+
  theme(aspect.ratio = 1,
        legend.position = "None",
        axis.text.x = element_text(colour = "black", size=13, face="italic", angle = 18, vjust =1, hjust =1),
