...
CRBD_leaves_top %>%
  mutate(Shade_tree_Canopy_Cover = scale(Shade_tree_Canopy_Cover)) -> CRBD_leaves_top
CRBD_leaves_top%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_Canopy_Cover*distance   + (1|sample_code)"),
                    zifrml = formula("~1")) -> mod_severity_CRBD_leaves_top

CRBD_leaves_top%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_Canopy_Cover*distance   + Coffee_Variety +
                                            (1|sample_code)"),
...
CRBD_leaves_top%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_Canopy_Cover*distance   +
                                            (1|sample_code)"),
...
```{r}
# Anova
Anova(mod_severity_CRBD_leaves_top3) %>%
  round(2) %>%
  knitr::kable()

Anova(mod_severity_CRBD_leaves_top3, component = "zi") %>%
  knitr::kable()

### Pairwise comparison variety
pwc_CRBD <- emmeans(mod_severity_CRBD_leaves_top3, ~Coffee_Variety, component = 'response')
summary(pwc_CRBD)
...
### Variety
predict_response(mod_severity_CRBD_leaves_top3, c("Coffee_Variety"), type = "zero_inflated") %>%
  as.data.frame %>%
  left_join(compl_letter, by = join_by(x == Coffee_Variety)) %>%
  ggplot() +
  geom_pointrange(aes(x = x, y = predicted, ymin = conf.low, ymax = conf.high,), 
...
```{r}
summary(mod_severity_CRBD_leaves_top3)

predict_response(mod_severity_CRBD_leaves_top3, c("distance [all]","Shade_tree_species"), type = "zero_inflated") %>% 
  as.data.frame %>%
  mutate(type = "fixed + zif") -> dist_shade_zif
predict_response(mod_severity_CRBD_leaves_top3, c("distance [all]","Shade_tree_species"), type = "fixed") %>%
  as.data.frame %>%
  mutate(type = "fixed") %>%
...
predict_response(mod_severity_CRBD_leaves_top3, c("distance [all]","Shade_tree_species"), type = "fixed") %>%plot

predict_response(mod_severity_CRBD_leaves_top3, c("Coffee_Variety"), type = "zero_inflated") %>%
  as.data.frame %>%
  mutate(type = "fixed + zif") -> coffee_var_zif
predict_response(mod_severity_CRBD_leaves_top3, c("Coffee_Variety"), type = "fixed") %>%
  as.data.frame %>%
  mutate(type = "fixed") %>%
...
```{r}
# Select model
MuMIn::AICc(mod_severity_CWD_top,mod_severity_CWD_top2, mod_severity_CWD_top3, mod_severity_CWD_top4) %>%
    knitr::kable()
# Anova
...
# Distance
predict_response(mod_severity_CWD_top, c("distance[all]"), type = "fixed") %>%
  as.data.frame %>%
  ggplot() +
  geom_ribbon(aes(x = x, y = predicted, ymin = conf.low, ymax = conf.high), alpha= 0.5) +
  geom_line(aes(x = x, y = predicted)) +
...
CLR_low %>%
  mutate(Shade_tree_Canopy_Cover = scale(Shade_tree_Canopy_Cover)) -> CLR_low

CLR_low %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_Canopy_Cover*distance + 
                                            (1|Section_of_Kaweri/sample_code)"),
                    zifrml =  formula("~1")) -> mod_severity_CLR_low_cover
CLR_low %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_Canopy_Cover*distance   + Coffee_Variety + 
                                            (1|Section_of_Kaweri/sample_code)"),
                              zifrml = formula("~1")) -> mod_severity_CLR_low2_cover
CLR_low %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_Canopy_Cover*distance    + Coffee_Variety+
                                            (1|Section_of_Kaweri/sample_code)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CLR_low3_cover
CLR_low %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_Canopy_Cover*distance    + 
                                            (1|Section_of_Kaweri/sample_code)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CLR_low4_cover
AIC(mod_severity_CLR_low3_cover, mod_severity_CLR_low3)
```
...
# Anova
Anova(mod_severity_CLR_low3_cover) %>%
  round(2) %>%
  knitr::kable()
Anova(mod_severity_CLR_low3_cover,component = "zi") %>%
  round(2) %>%
  knitr::kable()
summary(mod_severity_CLR_low3_cover)
