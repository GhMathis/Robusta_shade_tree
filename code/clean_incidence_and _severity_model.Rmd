...
```{r}
data <- read_xlsx("R_PhD_Data_corrected.xlsx")
...
glmmTMB(formula = severity ~ Shade_tree_species*distance + portion + log_total_count + Coffee_Variety+
                                            ...
                                            ziformula = ~Coffee_Variety*Shade_tree_species,
                                            
                                            family = binomial,
                                            weights = total_count,
                                            data = data_severity_CRBD_leaves) -> mod_severity_CRBD

res_severity_CRBD <- DHARMa::simulateResiduals(mod_severity_CRBD, plot = TRUE)


testQuantiles(mod_severity_CRBD)
testDispersion(res_severity_CRBD)
testZeroInflation(res_severity_CRBD)

model.frame(mod_severity_CRBD) -> mod_frame_CRBD
par(mfrow = c(2, 2))
plotResiduals(res_severity_CRBD, form = mod_frame_CRBD$portion)
plotResiduals(res_severity_CRBD, form = mod_frame_CRBD$distance, quantreg = T)
plotResiduals(res_severity_CRBD, form = mod_frame_CRBD$Shade_tree_species)
plotResiduals(res_severity_CRBD, form = mod_frame_CRBD$log_total_coun, quantreg = T)
par(mfrow = c(1, 1))
plotResiduals(res_severity_CRBD, form = data_severity_CRBD_leaves$Coffee_Variety)
```

```{r}
plot(allEffects(mod_severity_CRBD))

Anova(mod_severity_CRBD)
summary(mod_severity_CRBD)
```
...
```{r}
predict_response(mod_severity_CRBD_leaves_top4, c("distance [all]","Shade_tree_species"), type = "zero_inflated", bias_correction = T) %>% 
  as.data.frame %>%
  mutate(type = "fixed + zif") -> dist_shade_zif
predict_response(mod_severity_CRBD_leaves_top4, c("distance [all]","Shade_tree_species"), type = "fixed",  bias_correction = T) %>%
  as.data.frame %>%
  mutate(type = "fixed") %>%
  ...
  scale_fill_simpsons() +
  main_theme +
  labs(x= "Distance (m)", y = "Severity")

predict_response(mod_severity_CRBD_leaves_top4, c("Coffee_Variety"), type = "zi_prob",  bias_correction = T) %>%
  as.data.frame %>%
  ggplot() +
  ...
  scale_fill_simpsons() +
  main_theme +
  labs(x= "Coffee Variety", y = "severity")



# predict_response(mod_severity_CRBD_leaves_top3, c("distance [all]","Shade_tree_species"), type = "fixed",  bias_correction = T) %>%
#   plot
# 
# predict_response(mod_severity_CRBD_leaves_top3, c("Coffee_Variety"), type = "zero_inflated",  bias_correction = T) %>%
#   as.data.frame %>%
#   mutate(type = "fixed + zif") -> coffee_var_zif
# predict_response(mod_severity_CRBD_leaves_top3, c("Coffee_Variety"), type = "fixed",  bias_correction = T) %>%
#   as.data.frame %>%
#   mutate(type = "fixed") %>%
...
# 
# predict_response(mod_severity_CRBD_leaves_top3, c("log_total_count [all]"), type = "zero_inflated",  bias_correction = T) %>%
#   as.data.frame %>%
#   mutate(type = "fixed + zif") -> log_total_count_zif
# predict_response(mod_severity_CRBD_leaves_top3, c("log_total_count [all]"), type = "fixed",  bias_correction = T) %>%
#   as.data.frame %>%
#   mutate(type = "fixed") %>%
...
# Models
data_severity_CRBD_leaves %>%
  filter(portion =="middle") -> CRBD_leaves_mid

CRBD_leaves_mid%>%
  severity.mod.func -> mod_severity_CRBD_leaves_mid

CRBD_leaves_mid%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count  + Coffee_Variety +
                                            (1|sample_code/branch)")) -> mod_severity_CRBD_leaves_mid2

CRBD_leaves_mid%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count  + Coffee_Variety+
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CRBD_leaves_mid3
CRBD_leaves_mid%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count  +
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CRBD_leaves_mid4
...
```{r}
CRBD_leaves_mid %>%
  plot.effect.model(mod_severity_CRBD_leaves_mid4)
```
...
# Models
data_severity_CRBD_leaves %>%
  filter(portion =="lower") -> CRBD_leaves_low
CRBD_leaves_low%>%
  severity.mod.func -> mod_severity_CRBD_leaves_low
CRBD_leaves_low%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety +
                                            (1|sample_code/branch)")) -> mod_severity_CRBD_leaves_low2
CRBD_leaves_low%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety +
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CRBD_leaves_low3
CRBD_leaves_low%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CRBD_leaves_low4
CRBD_leaves_low%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety*Shade_tree_species")) -> mod_severity_CRBD_leaves_low5
CRBD_leaves_low%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety +
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety*Shade_tree_species")) -> mod_severity_CRBD_leaves_low6
```
...
CRBD_leaves_low %>%
  plot.effect.model(mod_severity_CRBD_leaves_low)

CRBD_leaves_low %>%
  plot.effect.model(mod_severity_CRBD_leaves_low6)
```
...
glmmTMB(formula = severity ~ Shade_tree_species*distance + portion +
                                            (1|sample_code/branch),
                                            ziformula = ~Coffee_Variety,
                                            family = binomial,
                                            weights = total_count,
                                            data = data_severity_CRBD_berries) -> mod_severity_CRBD_berries

res_severity_CRBD_berries <- DHARMa::simulateResiduals(mod_severity_CRBD_berries, plot = TRUE)


testQuantiles(mod_severity_CRBD_berries)
testDispersion(res_severity_CRBD_berries)
testZeroInflation(res_severity_CRBD_berries)

model.frame(mod_severity_CRBD_berries) -> mod_frame_CRBD_berries
par(mfrow = c(2, 2))
plotResiduals(res_severity_CRBD_berries, form = mod_frame_CRBD_berries$portion)
plotResiduals(res_severity_CRBD_berries, form = mod_frame_CRBD_berries$distance, quantreg = T)
plotResiduals(res_severity_CRBD_berries, form = mod_frame_CRBD_berries$Shade_tree_species)
plotResiduals(res_severity_CRBD_berries, form = mod_frame_CRBD_berries$log_total_count, quantreg = T)
par(mfrow = c(1, 1))
plotResiduals(res_severity_CRBD_berries, form = data_severity_CRBD_berries$Coffee_Variety)
```

```{r}
plot(allEffects(mod_severity_CRBD_berries))

Anova(mod_severity_CRBD_berries)
summary(mod_severity_CRBD_berries)
```
...
# Models
data_severity_CRBD_berries %>%
  filter(portion =="middle") -> CRBD_berries_mid
CRBD_berries_mid %>%
  severity.mod.func -> mod_severity_CRBD_berries_mid
CRBD_berries_mid %>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety +
                                            (1|sample_code/branch)")) -> mod_severity_CRBD_berries_mid2
CRBD_berries_mid %>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety +
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CRBD_berries_mid3
CRBD_berries_mid %>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CRBD_berries_mid4
...
```{r}
CRBD_berries_mid %>%
  plot.effect.model(mod_severity_CRBD_berries_mid4)
```
...
# Models
data_severity_CRBD_berries %>%
  filter(portion =="lower") -> CRBD_berries_low

CRBD_berries_low %>%
  severity.mod.func -> mod_severity_CRBD_berries_low

CRBD_berries_low %>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety +
                                            (1|sample_code/branch)")) -> mod_severity_CRBD_berries_low2

CRBD_berries_low %>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety +
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CRBD_berries_low3
CRBD_berries_low %>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count +
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CRBD_berries_low4
CRBD_berries_low %>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count +Coffee_Variety +
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety*Shade_tree_species")) -> mod_severity_CRBD_berries_low5
```

```{r}
CRBD_berries_low %>%
  plot.effect.model(mod_severity_CRBD_berries_low5)
```
...
CWD_top%>%
  severity.mod.func -> mod_severity_CWD_top

CWD_top%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)")) -> mod_severity_CWD_top2
CWD_top%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)"),
...
```{r}
CWD_top %>%
  plot.effect.model(mod_severity_CWD_top)
```
...
# Models
data_severity_CWD %>%
  filter(portion =="middle") -> CWD_mid

CWD_mid%>%
  severity.mod.func -> mod_severity_CWD_mid

CWD_mid%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)")) -> mod_severity_CWD_mid2
CWD_mid%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CWD_mid3
CWD_mid%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CWD_mid4
```

```{r}
CWD_mid %>%
  plot.effect.model(mod_severity_CWD_mid)

CWD_mid %>%
  plot.effect.model(mod_severity_CWD_mid2)
```
...
# Models
data_severity_CWD %>%
  filter(portion =="lower") -> CWD_low

CWD_low%>%
  severity.mod.func -> mod_severity_CWD_low

CWD_low%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)")) -> mod_severity_CWD_low2
CWD_low%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CWD_low3

CWD_low%>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CWD_low4

CWD_low%>%
  severity.mod.func(frml = formula("severity ~ log_total_count +
                                            (1|sample_code/branch)"),
                              zifrml = formula("~1")) -> mod_severity_CWD_low5

```

```{r}
CWD_low %>%
  plot.effect.model(mod_severity_CWD_low)
CWD_low %>%
  plot.effect.model(mod_severity_CWD_low5)
```
...
Zif is dependent on the shade tree species
...
glmmTMB(formula = severity ~ Shade_tree_species*distance + portion*Coffee_Variety + log_total_count  + 
                                            (1|sample_code/branch),
                                            ziformula = ~Coffee_Variety*portion,
                                            family = betabinomial,
                                            weights = total_count,
                                            data = data_severity_CLR) -> mod_severity_CLR

res_severity_CLR <- DHARMa::simulateResiduals(mod_severity_CLR, plot = TRUE)


testQuantiles(mod_severity_CLR)
testDispersion(res_severity_CLR)
testZeroInflation(res_severity_CLR)

model.frame(mod_severity_CLR) -> mod_frame_CLR
par(mfrow = c(2, 2))
plotResiduals(res_severity_CLR, form = mod_frame_CLR$portion)
plotResiduals(res_severity_CLR, form = mod_frame_CLR$distance, quantreg = T)
plotResiduals(res_severity_CLR, form = mod_frame_CLR$Shade_tree_species)
plotResiduals(res_severity_CLR, form = mod_frame_CLR$log_total_count, quantreg = T)
par(mfrow = c(1, 1))
plotResiduals(res_severity_CLR, form = data_severity_CLR$Coffee_Variety, quantreg = T)
```


```{r}
plot(allEffects(mod_severity_CLR))

Anova(mod_severity_CLR)
summary(mod_severity_CLR)
```
...
# Models
data_severity_CLR %>%
  filter(portion =="Top") -> CLR_top

CLR_top %>%
  severity.mod.func-> mod_severity_CLR_top
CLR_top %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~1")) -> mod_severity_CLR_top2
CLR_top %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count  + Coffee_Variety+
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CLR_top3
CLR_top %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count  + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CLR_top4
```

```{r}
CLR_top %>%
  plot.effect.model(mod_severity_CLR_top4)
```
...
# Models
data_severity_CLR %>%
  filter(portion =="middle") -> CLR_mid

CLR_mid %>%
  severity.mod.func-> mod_severity_CLR_mid
CLR_mid %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~1")) -> mod_severity_CLR_mid2
CLR_mid %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count  + Coffee_Variety+
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CLR_mid3
CLR_mid %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count  + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~Coffee_Variety")) -> mod_severity_CLR_mid4

CLR_mid %>%
 severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance  + log_total_count + Coffee_Variety + 
                                            (1|sample_code/branch)"),
                              zifrml = formula("~0")) -> mod_severity_CLR_mid5
```

```{r}
CLR_mid %>%
  plot.effect.model(mod_severity_CLR_mid2)

CLR_mid %>%
  plot.effect.model(mod_severity_CLR_mid5)
```
