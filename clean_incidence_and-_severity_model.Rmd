...
predict_response(mod_binary_incidence_CRBD_variety, c("Coffee_Variety"), type = "fixed") %>%
  as.data.frame %>%
  ggplot() +
...
pwc <-  emmeans::emmeans(mod_binary_incidence_CWD,
                ~ Coffee_Variety)
pwc

predict_response(mod_binary_incidence_CWD, c("Coffee_Variety"), type = "fixed") %>%
  as.data.frame %>%
  ggplot() +
  geom_pointrange(aes(x = x, y = predicted, ymin = conf.low, ymax = conf.high), 
                  linetype =1, cex = 0.5, linewidth = 1, position = position_dodge(width = .2)) +
...
predict_response(mod_binary_incidence_CLR_variety, c("Coffee_Variety"), type = "fixed") %>%
  as.data.frame %>%
  ggplot() +
  geom_pointrange(aes(x = x, y = predicted, ymin = conf.low, ymax = conf.high), 
                  linetype =1, cex = 0.5, linewidth = 1, position = position_dodge(width = .2)) +
...
data_severity %>%
  ...
  portion %in% c("Top", "lower")) %>%
  select(severity, sample_code, disease,portion,branch) %>%
  pivot_wider(names_from = disease, values_from = severity) -> disease_corr_df

disease_corr_df %>%
  filter(portion == "lower") %>%
  select(CWD, CLR, CRBD) %>%
  ggpairs()
...
CRBD_berries_top %>%
  severity.mod.func(frml = formula("severity ~ Shade_tree_species*distance   + (1|sample_code/branch)"),
...
res_severity_temp <- DHARMa::simulateResiduals(mod_severity_CRBD_berries_top4, plot = TRUE)
plotResiduals(res_severity_temp, form = CRBD_berries_top$Section_of_Kaweri)
```
...
res_severity_temp <- DHARMa::simulateResiduals(mod_severity_CLR_low3, plot = TRUE)
